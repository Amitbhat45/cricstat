...
@Composable
fun TestData2(text:String): Deferred<Pair<List<String>,List<String>>>{
    ...
    return remember {
        CoroutineScope(Dispatchers.IO).async {
            try {
                ...
                // Connect to the URL
                val doc: Document = Jsoup.connect("http://www.cricmetric.com/playerstats.py?player=${text.split(" ")[0]}+${text.split(" ")[1]}&role=batsman&format=Test&groupby=year&playerStatsFilters=on&start_date=2002-01-01&end_date=$formattedDate&start_over=0&end_over=9999&max_innings=1000")
                    .timeout(100000)
                ...
                for (row in foot) {
                    ...
                    allYearStat2.addAll(rowValues)  // Add row values to the list
                }
                ...
                dataByYear2.add(allYearStat2)
                val size=dataByYear2.size
                // Log the data

                val firstRow = dataByYear2[size-1]
                return@async Pair(firstRow,allYearStat2)
